...
form?.addEventListener('submit', async (e)=>{
  ...
  try {
    ...
    console.log('[SUBMIT] 5. Refrescando token...');
    ...
    try {
      const { data: freshSession } = await Promise.race([
        sb.auth.getSession(),
    ...
    if (!accessToken) {
      showToast('Sesión expirada. Cierra sesión e inicia de nuevo.', 'error'); return;
    }
    ...
    console.log('[SUBMIT] 6. Subiendo comprobante via XHR...');
    ...
    let archivo_url = null, archivo_mime = null;
    {
      ...
      const uploadResult = await new Promise((resolve, reject) => {
        ...
        xhr.upload.onprogress = (e) => {
          if (e.lengthComputable && submitBtn) {
            ...
            submitBtn.innerHTML = `<span class="verifying-spinner"></span> Subiendo ${pct}%…`;
          }
        };
      ...
      if (uploadResult.error) {
        showToast('No se pudo subir el archivo: ' + uploadResult.error, 'error'); return;
      }
