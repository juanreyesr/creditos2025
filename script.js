...
function setChip(el, text, mode){
  ...
  if(el === stErr) el.classList.toggle('hidden', !text || text === '—');
}

/* ========= Manejo de errores global ========= */
window.addEventListener('error', (e)=>{
  console.error('[GlobalError]', e?.error || e?.message);
  setChip(stErr, 'JS: ' + (e?.message || 'error'), 'err');
});
window.addEventListener('unhandledrejection', (e)=>{
  console.error('[UnhandledRejection]', e?.reason);
  setChip(stErr, 'Promise: ' + (e?.reason?.message || 'error'), 'err');
});
...
/* ========= Carga robusta del SDK Supabase ========= */
async function ensureSupabaseSDK(){
  if (window.supabase && window.supabase.createClient) {
    setChip(stSDK, 'SDK: listo', 'ok');
    return true;
  }
  // Si el <script> con defer aún no ejecutó, esperamos un poco:
  for (let i=0;i<80;i++){ // ~2s
    if (window.supabase && window.supabase.createClient){ setChip(stSDK, 'SDK: listo', 'ok'); return true; }
    await new Promise(r=>setTimeout(r,25));
  }
  // Carga dinámica de respaldo:
  ...
  setChip(stSDK, ok ? 'SDK: listo' : 'SDK: no cargó', ok ? 'ok' : 'err');
  return ok;
}
...
function getSupabaseClient(){
  try{
    const url = window.SB_URL; const key = window.SB_KEY;
    ...
    setChip(stSB, 'createClient error', 'err');
    return null;
  }
}

/* ========= Test rápido de conectividad ========= */
async function pingSupabase(sb){
  try {
    // health de auth
    const url = (window.SB_URL || '').replace(/\/+$/,'') + '/auth/v1/health';
    const res = await fetch(url, { mode:'cors' });
    if (!res.ok) throw new Error('HTTP '+res.status);
    setChip(stSB, 'Supabase ON', 'ok');
  } catch (e) {
    console.warn('No hay conectividad con Supabase', e);
    setChip(stSB, 'Sin conexión a Supabase', 'err');
  }
}
...
/* ========= Uploader ========= */
(function(){
  const now=new Date(); if(fechaEl) fechaEl.max = now.toISOString().slice(0,10);
...
function handleFile(file){
  ...
  if(file.type==='application/pdf'){ const url=URL.createObjectURL(file); const emb=document.createElement('embed'); emb.src=url; emb.type='application/pdf'; emb.className='pdf'; preview.appendChild(emb); }
  else { const img=document.createElement('img'); img.className='thumb'; img.alt='Vista previa'; img.src=URL.createObjectURL(file); preview.appendChild(img); }
}

/* ========= Carga de SDK + cliente + sesión inicial ========= */
(async function boot(){
  // 1) SDK
  const okSDK = await ensureSupabaseSDK();
  if(!okSDK){ showToast('No se pudo cargar el SDK de Supabase. Revisa tu conexión.', 'error'); return; }

  // 2) Credenciales + cliente
  const sb = getSupabaseClient();
  if(!sb){ showToast('Credenciales Supabase faltan o inválidas.', 'error'); return; }
  await pingSupabase(sb);
...
gateLogin?.addEventListener('click', ()=> openModal(authModal));
gateSignup?.addEventListener('click', ()=> openModal(authModal));
...
doRecover?.addEventListener('click', async ()=>{
  const sb = getSupabaseClient(); if(!sb){ showToast('Supabase no disponible.','error'); return; }
  const email = (authEmail?.value||'').trim();
  if(!email){ showToast('Ingresa tu email y pulsa recuperar.', 'warn'); return; }
  try{
    const redirectTo = `${location.origin}/auth-callback.html`;
...
async function refreshAdminState(){
  ...
  __IS_ADMIN = isAdmin;
  document.getElementById('adminBadge').hidden = !__IS_ADMIN;
  const rb = document.getElementById('adminRoleBadge'); if(rb) rb.hidden = !__IS_ADMIN;
...
openAdminBtn?.addEventListener('click', async ()=>{
  ...
  if(!__IS_ADMIN){ adminBody.hidden = true; adminAuth.hidden = false; adminNoRights.hidden = false; }
  else { adminNoRights.hidden = true; }
});
...
adminLogin?.addEventListener('click', async (ev)=>{
  ...
  adminAuth.hidden = true; adminBody.hidden = false; startAdminSession();
...
document.getElementById('adminTable')?.addEventListener('click', async (e)=>{
  ...
  if(action==='pdf'){
    ...
    const ok = confirm(`¿Eliminar (soft delete) el registro ${corr}?`); if(!ok) return;
    const { error: upErr } = await sb.from('registros').update({ deleted_at: new Date().toISOString() }).eq('id', id).is('deleted_at', null);
    if(upErr){ console.error(upErr); showToast('No se pudo eliminar (permisos/RLS).','error'); return; }
...
exportCSVBtn?.addEventListener('click', async ()=>{
  if(adminBody.hidden || Date.now() >= (adminSessionEnd||0)) return showToast('Inicie sesión admin','error');
  const sb = getSupabaseClient(); if(!sb){ showToast('Supabase no disponible.','error'); return; }
...
const QR_X = 450, QR_Y = 64, QR_SIZE = 96, LOGO_BELOW_GAP = 12;
...
async function getQrDataUrl(text, size=96){
  ...
  return new Promise((resolve)=>{
    ...
    if (canvas){ try { return resolve(canvas.toDataURL('image/png')); } catch { return resolve(null); } }
    if (img){
...
async function generarConstanciaPDF(rec){
  ...
  try {
    ...
    if (logo) {
      const logoY = QR_Y + QR_SIZE + 12;
      doc.addImage(logo, 'PNG', QR_X, logoY, 96, 96);
    }
  } catch {}
...
async function loadAndRender(){
  ...
  let rows = null;
  try {
    const { data, error } = await sb.from('registros').select('*').eq('usuario_id', session.session.user.id).is('deleted_at', null).order('created_at',{ascending:false});
    if (error) throw error;
    rows = data;
  } catch {
    const { data, error } = await sb.from('registros').select('*').eq('usuario_id', session.session.user.id).order('created_at',{ascending:false});
