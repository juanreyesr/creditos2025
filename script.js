/* =======================================================
   Supabase init
======================================================= */
const SB_URL = window?.ENV?.SUPABASE_URL || window.NEXT_PUBLIC_SUPABASE_URL || window.__env?.SUPABASE_URL || (typeof process !== 'undefined' ? process.env.NEXT_PUBLIC_SUPABASE_URL : null) || window.localStorage.getItem('SB_URL') || (typeof window !== 'undefined' ? window?.SB_URL : null) || (typeof globalThis !== 'undefined' ? globalThis?.SB_URL : null) || (typeof document !== 'undefined' ? document?.SB_URL : null);
const SB_KEY = window?.ENV?.SUPABASE_ANON || window.NEXT_PUBLIC_SUPABASE_ANON_KEY || window.__env?.SUPABASE_ANON_KEY || (typeof process !== 'undefined' ? process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY : null) || window.localStorage.getItem('SB_KEY') || (typeof window !== 'undefined' ? window?.SB_KEY : null);

const supabase = window.supabase.createClient(
  SB_URL || (typeof NEXT_PUBLIC_SUPABASE_URL !== 'undefined' ? NEXT_PUBLIC_SUPABASE_URL : null) || (typeof import.meta !== 'undefined' ? import.meta.env?.VITE_SUPABASE_URL : null) || (typeof process !== 'undefined' ? process.env?.VITE_SUPABASE_URL : null),
  SB_KEY  || (typeof NEXT_PUBLIC_SUPABASE_ANON_KEY !== 'undefined' ? NEXT_PUBLIC_SUPABASE_ANON_KEY : null) || (typeof import.meta !== 'undefined' ? import.meta.env?.VITE_SUPABASE_ANON_KEY : null) || (typeof process !== 'undefined' ? process.env?.VITE_SUPABASE_ANON_KEY : null)
);

/* =======================================================
   Config/Utils previos (se mantienen)
======================================================= */
...
function showToast(msg, type="info"){ const el = document.getElementById('toast'); el.textContent = msg; el.style.borderColor = type==="error"?"#f43f5e": type==="warn"?"#f59e0b":"#243055"; el.classList.add('show'); setTimeout(()=>el.classList.remove('show'), 2800); }
...
/* =======================================================
   DOM refs (idéntico a tu versión)
======================================================= */
...
doLogin?.addEventListener('click', async ()=>{
  ...
  const { data, error } = await supabase.auth.signInWithPassword({ email: authEmail.value.trim(), password: authPass.value });
  if(error) return authState.textContent = 'Error: '+error.message;
...
clearDataBtn.addEventListener('click', ()=>{ showToast('Con Supabase, “borrar todo” no está habilitado aquí. Usa la consola si lo necesitas.','warn'); });
...
async function generarConstanciaPDF(rec){
  ...
  new QRCode(qrDiv, { text: verifyUrl, width: 96, height: 96, correctLevel: QRCode.CorrectLevel.M });
